...
public class GuiRepulsiveCrusher extends GuiContainer
{
    ...
    private TileEntityRepulsiveCrusher furnaceInventory;
    private static final ResourceLocation background_texture = new ResourceLocation(ModInformation.GUITEXTUREPATH + "/" + ModInformation.REPULSIVECRUSHER_GUI + ".png");
    ...
    public GuiRepulsiveCrusher(InventoryPlayer par1InventoryPlayer, TileEntityRepulsiveCrusher par2TileEntityFurnace)
    {
        super(new ContainerRepulsiveCrusher(par1InventoryPlayer, par2TileEntityFurnace));
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().repCrusherRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().repCrusherRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), RepulsiveCrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, PowerHandler.RepCrusherSpeed);
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), RepulsiveCrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), RepulsiveCrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = PowerHandler.RepCrusherSpeed * PowerHandler.RepCrusherConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.RepCrusherConsump + " EE/tick";

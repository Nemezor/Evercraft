...
public class GuiParticleMerger extends GuiContainer
{
    ...
    private TileEntityParticleMerger furnaceInventory;
    private static final ResourceLocation background_texture = new ResourceLocation(ModInformation.GUITEXTUREPATH + "/" + ModInformation.PARTICLEMERGER_GUI + ".png");
    ...
    public GuiParticleMerger(InventoryPlayer par1InventoryPlayer, TileEntityParticleMerger par2TileEntityFurnace)
    {
        super(new ContainerParticleMerger(par1InventoryPlayer, par2TileEntityFurnace));
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().particleMergerRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().particleMergerRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), ParticleMergerRecipes.compacting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, ParticleMergerRecipes.compacting().getParticleMergerRecipeProcessSpeed(this.recipe_inputs.get(this.recipeCount)));
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), ParticleMergerRecipes.compacting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), ParticleMergerRecipes.compacting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = processSpeed * PowerHandler.ParticleMergConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.ParticleMergConsump + " EE/tick";

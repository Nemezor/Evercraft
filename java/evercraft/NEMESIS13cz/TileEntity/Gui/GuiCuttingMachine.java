...
public class GuiCuttingMachine extends GuiContainer
{
    ...
    private TileEntityCuttingMachine furnaceInventory;
    private static final ResourceLocation background_texture = new ResourceLocation(ModInformation.GUITEXTUREPATH + "/" + ModInformation.PLATECUTTER_GUI + ".png");
    ...
    public GuiCuttingMachine(InventoryPlayer par1InventoryPlayer, TileEntityCuttingMachine par2TileEntityFurnace)
    {
        super(new ContainerCuttingMachine(par1InventoryPlayer, par2TileEntityFurnace));
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().cuttingMachineRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().cuttingMachineRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), CuttingMachineRecipes.cutting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, CuttingMachineRecipes.cutting().CuttingRecipesProcessSpeed(this.recipe_inputs.get(this.recipeCount)));
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), CuttingMachineRecipes.cutting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), CuttingMachineRecipes.cutting().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = processSpeed * PowerHandler.CuttingMachineConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.CuttingMachineConsump + " EE/tick";

...
public class GuiHammeringMachine extends GuiContainer
{
    ...
    private TileEntityHammeringMachine furnaceInventory;
    private static final ResourceLocation background_texture = new ResourceLocation(ModInformation.GUITEXTUREPATH + "/" + ModInformation.HAMMERINGMACHINE_GUI + ".png");
    ...
    public GuiHammeringMachine(InventoryPlayer par1InventoryPlayer, TileEntityHammeringMachine par2TileEntityFurnace)
    {
        super(new ContainerHammeringMachine(par1InventoryPlayer, par2TileEntityFurnace));
        this.furnaceInventory = par2TileEntityFurnace;
        this.addRecipes();
        this.recipeCount = -1;
    }

    private static MetaInterface plates;
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().hammeringMachineRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().hammeringMachineRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), HammeringMachineRecipes.hammering().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, HammeringMachineRecipes.hammering().HammeringRecipesProcessSpeed(this.recipe_inputs.get(this.recipeCount)));
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), HammeringMachineRecipes.hammering().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), HammeringMachineRecipes.hammering().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = processSpeed * PowerHandler.HammeringMachineConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.HammeringMachineConsump + " EE/tick";

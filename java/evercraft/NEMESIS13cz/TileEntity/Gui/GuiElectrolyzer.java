...
public class GuiElectrolyzer extends GuiContainer
{
    ...
    private TileEntityElectrolyzer furnaceInventory;
    ...
    public GuiElectrolyzer(InventoryPlayer par1InventoryPlayer, TileEntityElectrolyzer par2TileEntityFurnace)
    {
        super(new ContainerElectrolyzer(par1InventoryPlayer, par2TileEntityFurnace));
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().electrolyzerRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().electrolyzerRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), ElectrolyzerRecipes.electrolyzing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, PowerHandler.ElectrolyzerSpeed);
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), ElectrolyzerRecipes.electrolyzing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), ElectrolyzerRecipes.electrolyzing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = PowerHandler.ElectrolyzerSpeed * PowerHandler.ElectrolyzerConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.ElectrolyzerConsump + " EE/tick";

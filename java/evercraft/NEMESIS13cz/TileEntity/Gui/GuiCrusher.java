...
public class GuiCrusher extends GuiContainer
{
    ...
    public GuiCrusher(InventoryPlayer par1InventoryPlayer, TileEntityCrusher par2TileEntityMachine) {
        super(new ContainerCrusher(par1InventoryPlayer, par2TileEntityMachine));
	...
	private void addRecipes() {
		for (int i = 0; i < Evercraft.getEvercraft().crusherRecipes.length; i++) {
			this.registerRecipe((ItemStack) Evercraft.getEvercraft().crusherRecipes[i]);
		}
	}
    ...
    private void getRecipe(boolean mode, int mouseX, int mouseY, int posX1, int posY1, int posX2, int posY2) {
    	if (this.recipeCount > -1) {
	    	if (mode) {
	    		this.drawRecipeForeground(this.recipe_inputs.get(this.recipeCount).getDisplayName(), CrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).getDisplayName(), mouseX, mouseY, posX1, posY1, PowerHandler.CrusherSpeed);
	    	}else{
	    		this.drawRecipeBackground(this.recipe_inputs.get(this.recipeCount), CrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)), CrusherRecipes.crushing().getSmeltingResult(this.recipe_inputs.get(this.recipeCount)).stackSize, posX2, posY2);
	    	}
    	}
    }
    ...
    private void drawRecipeForeground(String inputName, String outputName, int mouseX, int mouseY, int x, int y, int processSpeed) {
    	if (this.recipeCount > -1) {
    		...
    		String cost = PowerHandler.CrusherSpeed * PowerHandler.CrusherConsump + " EE";
            this.fontRendererObj.drawString(cost, 214 - this.fontRendererObj.getStringWidth(cost) / 2, this.ySize - 138, 4210752);
    		String consumption = PowerHandler.CrusherConsump + " EE/tick";

...
public class TileEntityRepulsiveCrusher extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Repulsive Crusher";
    }
    ...
    {
        return this.furnaceCookTime * par1 / PowerHandler.RepCrusherSpeed;
    }
    ...
    {
        return this.power > PowerHandler.RepCrusherConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= PowerHandler.RepCrusherConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == PowerHandler.RepCrusherSpeed)
                {
            ...
            {
                flag1 = true;
                BlockRepulsiveCrusher.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    private boolean canStartSmelting()
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null)
        {
            ...
            {
                ItemStack itemstack = RepulsiveCrusherRecipes.crushing().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        if (this.power <= 0 || this.processedItem == null)
        {
            ...
            {
                ItemStack itemstack = RepulsiveCrusherRecipes.crushing().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && RepulsiveCrusherRecipes.crushing().getSmeltingResult(item) != null) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

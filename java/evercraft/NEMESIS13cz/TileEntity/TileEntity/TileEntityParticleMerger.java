...
public class TileEntityParticleMerger extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Pressurizer";
    }
    ...
    {
        return this.power > PowerHandler.ParticleMergConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= PowerHandler.ParticleMergConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = ParticleMergerRecipes.compacting().getParticleMergerRecipeProcessSpeed(processedItem);
            }else{
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == ParticleMergerRecipes.compacting().getParticleMergerRecipeProcessSpeed(processedItem))
                {
            ...
            {
                flag1 = true;
                BlockParticleMerger.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    private boolean canStartSmelting()
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null)
        {
            ...
            {
                ItemStack itemstack = ParticleMergerRecipes.compacting().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        if (this.power <= 0 || this.processedItem == null)
        {
            ...
            {
                ItemStack itemstack = ParticleMergerRecipes.compacting().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && ParticleMergerRecipes.compacting().getSmeltingResult(item) != null) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

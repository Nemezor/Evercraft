...
public class TileEntityHammeringMachine extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Hammering Machine";
    }
    ...
    {
        return this.power > PowerHandler.HammeringMachineConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= PowerHandler.HammeringMachineConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = HammeringMachineRecipes.hammering().HammeringRecipesProcessSpeed(processedItem);
            }else{
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == HammeringMachineRecipes.hammering().HammeringRecipesProcessSpeed(processedItem))
                {
            ...
            {
                flag1 = true;
                BlockHammeringMachine.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    private boolean canStartSmelting()
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null)
        {
            ...
            {
                ItemStack itemstack = HammeringMachineRecipes.hammering().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        if (this.power <= 0 || this.processedItem == null)
        {
            ...
            {
                ItemStack itemstack = HammeringMachineRecipes.hammering().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && HammeringMachineRecipes.hammering().getSmeltingResult(item) != null) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

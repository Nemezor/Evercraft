...
public class TileEntityAlloyFurnace extends TileEntityMachine implements ISidedInventory
{
     ...
     */
    public ItemStack[] furnaceItemStacks = new ItemStack[5];

    public int maxPower = PowerHandler.highCapacity;
    public int upgradeType;
    private static MachineUpgrades UPGR;
    
    private ItemStack[] processedItem = new ItemStack[2];
    
    public int S2CsyncProcessSpeed;
    
    public int S2CsyncPowerConsump;
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Alloy Furnace";
    }
    ...
    public void readFromNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            byte b0 = nbttagcompound1.getByte("Input");

            if (b0 >= 0 && b0 < 2)
            {
                this.processedItem[b0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
            }
        }
    ...
    public void writeToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        if (this.processedItem[0] != null && this.processedItem[1] != null) {

            NBTTagList nbttaglist2 = new NBTTagList();
            
            for (int i = 0; i < 2; ++i)
            {
                if (this.processedItem[i] != null)
                {
                    NBTTagCompound nbttagcompound1 = new NBTTagCompound();
                    nbttagcompound1.setByte("Input", (byte)i);
                    this.processedItem[i].writeToNBT(nbttagcompound1);
    ...
    {
        return this.power > S2CsyncPowerConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            if (processedItem[0] != null && processedItem[1] != null) {
            	S2CsyncPowerConsump = AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesPowerConsumption(processedItem);
            }else if(this.furnaceItemStacks[0] != null && this.furnaceItemStacks[1] != null && AlloyFurnaceRecipes.smelting().getSmeltingResult(new ItemStack[] {this.furnaceItemStacks[0], this.furnaceItemStacks[1]}) != null) {
            	S2CsyncPowerConsump = AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesPowerConsumption(new ItemStack[] {this.furnaceItemStacks[0], this.furnaceItemStacks[1]});
            }else{
            	S2CsyncPowerConsump = 0;
            }
        }
        
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= S2CsyncPowerConsump;
        }
        
        if (!this.worldObj.isRemote)
        {        	
            this.upgradeType = this.setUpgradeType(this.furnaceItemStacks[4]);
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesProcessSpeed(processedItem);
            }else{
            	S2CsyncProcessSpeed = 0;
            }
            
            if (this.furnaceCookTime == 0) {
            	if (this.setUpgradeType(this.furnaceItemStacks[4]) == UPGR.redstoneUpgradeID) {
            ...
            if (this.hasPower() && this.canStartSmelting() && flagRS && this.processedItem[0] == null && this.processedItem[1] == null)
            {
            	this.consumeInput();
            	++this.furnaceCookTime;
            	this.power -= AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesPowerConsumption(processedItem);
            }
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem[0] != null && this.processedItem[1] != null)
            {
                ++this.furnaceCookTime;

                if (this.furnaceCookTime == AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesProcessSpeed(processedItem))
                {
                    this.furnaceCookTime = 0;
                    this.smeltItem();
                	this.power -= AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesPowerConsumption(processedItem);
                    this.processedItem[0] = null;
                    this.processedItem[1] = null;
                    flag1 = true;
                }
            }
            ...
            {
                flag1 = true;
                BlockAlloyFurnace.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null || this.furnaceItemStacks[1] == null)
        {
            return false;
        }
            else
            {
            	int[] inputAmounts = AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesInputAmount(new ItemStack[] {this.furnaceItemStacks[0], this.furnaceItemStacks[1]});
            	if (this.furnaceItemStacks[0].stackSize < inputAmounts[0]) return false;
            	if (this.furnaceItemStacks[1].stackSize < inputAmounts[1]) return false;
                ItemStack itemstack = AlloyFurnaceRecipes.smelting().getSmeltingResult(new ItemStack[] {this.furnaceItemStacks[0], this.furnaceItemStacks[1]});
    ...
    {
        if (this.power <= 0 || this.processedItem[0] == null || this.processedItem[1] == null)
        {
            return false;
        }
            else
            {
                ItemStack itemstack = AlloyFurnaceRecipes.smelting().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[3] == null) return true;
                if (!this.furnaceItemStacks[3].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[3].stackSize + itemstack.stackSize;
    ...
    public void consumeInput() {
    	...
    	this.processedItem = new ItemStack[] {new ItemStack(this.furnaceItemStacks[0].getItem(), 1, this.furnaceItemStacks[0].getItemDamage()), new ItemStack(this.furnaceItemStacks[1].getItem(), 1, this.furnaceItemStacks[1].getItemDamage())};
    	
    	int[] inputAmounts = AlloyFurnaceRecipes.smelting().AlloyFurnaceRecipesInputAmount(this.processedItem);
        this.furnaceItemStacks[0].stackSize -= inputAmounts[0];

        if (this.furnaceItemStacks[0].stackSize <= 0)
        {
            this.furnaceItemStacks[0] = null;
        }
        this.furnaceItemStacks[1].stackSize -= inputAmounts[1];
    ...
    public boolean isMainTextureOn() {
    	ItemStack[] items = new ItemStack[] {this.furnaceItemStacks[0], this.furnaceItemStacks[1]};
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.furnaceItemStacks[1] != null && this.hasPower() && AlloyFurnaceRecipes.smelting().getSmeltingResult(items) != null) && (this.canContinueSmelting() || this.canStartSmelting()) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

...
public class TileEntityCuttingMachine extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Cutting Machine";
    }
    ...
    {
        return this.power > PowerHandler.CuttingMachineConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= PowerHandler.CuttingMachineConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = CuttingMachineRecipes.cutting().CuttingRecipesProcessSpeed(processedItem);
            }else{
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == CuttingMachineRecipes.cutting().CuttingRecipesProcessSpeed(processedItem))
                {
            ...
            {
                flag1 = true;
                BlockCuttingMachine.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    private boolean canStartSmelting()
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null)
        {
            ...
            {
                ItemStack itemstack = CuttingMachineRecipes.cutting().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        if (this.power <= 0 || this.processedItem == null)
        {
            ...
            {
                ItemStack itemstack = CuttingMachineRecipes.cutting().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && CuttingMachineRecipes.cutting().getSmeltingResult(item) != null) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

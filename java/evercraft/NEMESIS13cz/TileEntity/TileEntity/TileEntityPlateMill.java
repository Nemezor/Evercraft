...
public class TileEntityPlateMill extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Plate Mill";
    }
    ...
    {
        return this.power > PowerHandler.PlateMillConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && (this.isSmelting() || (!this.isFormed && this.furnaceItemStacks[0] != null && this.furnaceCookTime > 0))) {
        	this.power -= PowerHandler.PlateMillConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = PlateMillRecipes.plating().getPlateMillRecipeProcessSpeed(processedItem);
            }else{
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == PlateMillRecipes.plating().getPlateMillRecipeProcessSpeed(processedItem))
                {
                    this.furnaceCookTime = 0;
                    this.smeltItem();
                    this.processedItem = null;
            ...
            {
                flag1 = true;
                BlockPlateMill.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    {
	if (w.getBlock(x, y - 1, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x - 1, y - 1, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x + 1, y - 1, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x, y - 1, z - 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x, y - 1, z + 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x + 1, y - 1, z + 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x - 1, y - 1, z + 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x - 1, y - 1, z - 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x + 1, y - 1, z - 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x, y - 2, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x - 1, y - 2, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x + 1, y - 2, z) == Evercraft_Blocks.blockcasing2 && w.getBlock(x, y - 2, z - 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x, y - 2, z + 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x + 1, y - 2, z + 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x - 1, y - 2, z + 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x + 1, y - 2, z + 1) == Evercraft_Blocks.blockcasing1 && w.getBlock(x + 1, y - 2, z - 1) == Evercraft_Blocks.blockcasing1) { 
        return true;
    }else
        return false;
    }
    ...
    private boolean canStartSmelting()
    {
        ...
        if (!isFormed)
        {
            ...
            {
                ItemStack itemstack = PlateMillRecipes.plating().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        ...
        if (!isFormed)
        {
            ...
            {
                ItemStack itemstack = PlateMillRecipes.plating().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && PlateMillRecipes.plating().getSmeltingResult(item) != null && this.furnaceCookTime > 0) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

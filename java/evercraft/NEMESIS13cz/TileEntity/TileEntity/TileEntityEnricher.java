...
public class TileEntityEnricher extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Ore Separator";
    }
    ...
    {
        return this.power > PowerHandler.OreEnricherConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && this.isSmelting()) {
        	this.power -= PowerHandler.OreEnricherConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.furnaceCookTime > 0) {
            	S2CsyncProcessSpeed = EnricherRecipes.enriching().getProcessSpeed(processedItem);
            }else{
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == EnricherRecipes.enriching().getProcessSpeed(this.processedItem))
                {
            ...
            {
                flag1 = true;
                BlockEnricher.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
    ...
    private boolean canStartSmelting()
    {
        if (this.power <= 0 || this.furnaceItemStacks[0] == null)
        {
            ...
            {
                ItemStack itemstack = EnricherRecipes.enriching().getSmeltingResult(this.furnaceItemStacks[0]);
                if (itemstack == null) return false;
                ItemStack secOutput = EnricherRecipes.enriching().getSecondOutput(this.furnaceItemStacks[0]);
                if (secOutput != null && this.furnaceItemStacks[3] != null) {
                    if (!ItemStackHelper.areItemStacksEqual(this.furnaceItemStacks[3], secOutput)) return false;
                    if (this.furnaceItemStacks[3].stackSize + secOutput.stackSize > this.getInventoryStackLimit()) return false;
                }
                if (this.furnaceItemStacks[2] == null) return true;
                if (this.furnaceItemStacks[0].stackSize < EnricherRecipes.enriching().getInputAmount(this.furnaceItemStacks[0])) return false;
    ...
    private boolean canContinueSmelting()
    {
        if (this.power <= 0 || this.processedItem == null)
        {
            ...
            {
                ItemStack itemstack = EnricherRecipes.enriching().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                ItemStack secOutput = EnricherRecipes.enriching().getSecondOutput(this.processedItem);
                if (secOutput != null && this.furnaceItemStacks[3] != null) {
                    if (!ItemStackHelper.areItemStacksEqual(this.furnaceItemStacks[3], secOutput)) return false;
                    if (this.furnaceItemStacks[3].stackSize + secOutput.stackSize > this.getInventoryStackLimit()) return false;
                }
                if (this.furnaceItemStacks[2] == null) return true;
                if (this.processedItem.stackSize < EnricherRecipes.enriching().getInputAmount(this.processedItem)) return false;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
                return result <= getInventoryStackLimit() && result <= this.furnaceItemStacks[2].getMaxStackSize();
            }
    ...
    public void smeltItem()
    {
        ...
        {
            ItemStack itemstack = EnricherRecipes.enriching().getSmeltingResult(this.processedItem);
            ItemStack itemstack2 = EnricherRecipes.enriching().getSecondOutput(this.processedItem);
            
            boolean secondOutput;
            int percentage = EnricherRecipes.enriching().getPercentage(this.processedItem);
            int randNum = percentage > 0 ? this.rand.nextInt(101) : 0;
            secondOutput = randNum > 0 && randNum <= percentage;
    ...
    public void consumeInput() {
    	int in = EnricherRecipes.enriching().getInputAmount(this.furnaceItemStacks[0]);
    	
    	this.processedItem = new ItemStack(this.furnaceItemStacks[0].getItem(), in, this.furnaceItemStacks[0].getItemDamage());
    	
        this.furnaceItemStacks[0].stackSize -= in;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && EnricherRecipes.enriching().getSmeltingResult(item) != null) && (this.canContinueSmelting() || this.canStartSmelting()) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}

...
public class TileEntityStandardGenerator extends TileEntityGenerator implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Etheric Generator";
    }
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (flag != this.hasPower() || flagSent)
            {
                flag1 = true;
                BlockStandardGenerator.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    public static int getPower(ItemStack par0ItemStack)
    {
        ...
        else
        {
            ...
            if (item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.air) {
                ...
                if (block == Blocks.glowstone) return PowerHandler.FuelValue * 4 * PowerHandler.ethericGeneratorFuelMultiplier;
                if (block == Blocks.redstone_block) return (PowerHandler.FuelValue / 6) * 9 * PowerHandler.ethericGeneratorFuelMultiplier;
                
            }

            if (item == Items.glowstone_dust) return PowerHandler.FuelValue * PowerHandler.ethericGeneratorFuelMultiplier;
            if (item == Items.redstone) return PowerHandler.FuelValue / 6 * PowerHandler.ethericGeneratorFuelMultiplier;
            if (item == Evercraft_Items.redstonepowder) return PowerHandler.FuelValue / 24 * PowerHandler.ethericGeneratorFuelMultiplier;
            if (item == Evercraft_Items.enrichedredstone) return PowerHandler.FuelValue * 2 * PowerHandler.ethericGeneratorFuelMultiplier;
            return 0;
        }
    }
    /**

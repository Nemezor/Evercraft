...
public class TileEntityAdvancedElectrolyzer extends TileEntityMachine implements ISidedInventory
{
    ...
    {
        return this.hasCustomInventoryName() ? this.field_94130_e : "Advanced Electrolyzer";
    }
    ...
    {
        return this.furnaceCookTime * par1 / PowerHandler.AdvElectrolyzerSpeed;
    }
    ...
    {
        return this.power > PowerHandler.AdvElectrolyzerConsump - 1;
    }
    ...
    public void updateEntity() {
        ...
        if (this.hasPower() && (this.isSmelting() || (!this.isFormed && this.furnaceItemStacks[0] != null && this.furnaceCookTime > 0))) {
        	this.power -= PowerHandler.AdvElectrolyzerConsump;
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.hasPower() && this.canContinueSmelting() && flagRS && this.processedItem != null)
            {
                ...
                if (this.furnaceCookTime == PowerHandler.AdvElectrolyzerSpeed)
                {
                    this.furnaceCookTime = 0;
                    this.smeltItem();
                    this.processedItem = null;
            ...
            {
                flag1 = true;
                BlockAdvancedElectrolyzer.updateFurnaceBlockState(this.isMainTextureOn(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    {
    			// y = 0
			if (w.getBlock(x + 1, y - 1, z) == Evercraft_Blocks.blockcasing3 && w.getBlock(x + 1, y - 1, z + 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x + 1, y - 1, z - 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x, y - 1, z) == Evercraft_Blocks.blockcasing3 && w.getBlock(x, y - 1, z + 1) == Evercraft_Blocks.blockcasing3 && w.getBlock(x, y - 1, z - 1) == Evercraft_Blocks.blockcasing3 && w.getBlock(x - 1, y - 1, z) == Evercraft_Blocks.blockcasing3 && w.getBlock(x - 1, y - 1, z + 1) == Evercraft_Blocks.blockcasing2 && w.getBlock(x - 1, y - 1, z - 1) == Evercraft_Blocks.blockcasing2) {
            	return true;
            }else
        return false;
    }
    ...
    private boolean canStartSmelting()
    {
        ...
        if (!isFormed)
        {
            ...
            {
                ItemStack itemstack = AdvancedElectrolyzerRecipes.advancedElectrolyzing().getSmeltingResult(this.furnaceItemStacks[0]);
    ...
    private boolean canContinueSmelting()
    {
        ...
        if (!isFormed)
        {
            ...
            {
                ItemStack itemstack = AdvancedElectrolyzerRecipes.advancedElectrolyzing().getSmeltingResult(this.processedItem);
                if (itemstack == null) return false;
                if (this.furnaceItemStacks[2] == null) return true;
                if (!this.furnaceItemStacks[2].isItemEqual(itemstack)) return false;
                int result = furnaceItemStacks[2].stackSize + itemstack.stackSize;
    ...
    public boolean isMainTextureOn() {
    	...
    	if (this.isSmelting() || (this.furnaceItemStacks[0] != null && this.hasPower() && AdvancedElectrolyzerRecipes.advancedElectrolyzing().getSmeltingResult(item) != null && this.furnaceCookTime > 0) && flagRS) {
    		return true;
    	}else{
    		return false;
    	}
